...
export default function MyRoster() {
  ...
  const spent = totalBudget - myBudget
  const budgetPct = Math.max(0, Math.min(100, (myBudget / totalBudget) * 100))
      ...
      <div className="bg-[#1c1f2e] rounded-lg p-3 mb-3">
        <div className="flex justify-between text-sm mb-2">
          <span className="text-slate-300">
            Budget: <span className="font-mono font-medium">${myBudget}</span>
          </span>
        ...
        <div className="h-2 bg-[#0f1117] rounded-full overflow-hidden mb-2">
          <div
            className={`h-full rounded-full transition-all ${getBudgetColor(myBudget, totalBudget)}`}
