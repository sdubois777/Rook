...
- **QB replacement anchor — RESOLVED:** the QB anchor read too high not because of the rank/pool but because `derive_anchors` measured replacement/elite on raw `season_ppg` while `forward_value` scales the recency-blended in-season LEVEL. Fixed by deriving anchors on that SAME level basis (`inseason_level_by_position`), for all positions — top QBs now read as starters (e.g. Mahomes 10.4 → 21.4). Anchor-independent (no circularity); the #170 factors still move individual players within the band.
