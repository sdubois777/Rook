...
- **QB replacement anchor (#160)** derives to ~16.1, *above* the median starting QB, so starting QBs read in the low band even after the soft floor (`_REPLACEMENT_FLOOR`) restored their ordering — re-derive the QB anchor next (an anchor-derivation question, not a clamp question; the soft floor only fixed the all-0 collapse).
